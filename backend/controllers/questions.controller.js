...
const enviarRespuestas = async (req, res) => {
  ...
  if (!Array.isArray(answers) || !userName) {
    return res
      .status(400)
      .json({
        message: "Datos incompletos. Debes enviar 'userName' y 'answers'.",
      });
  }
  ...
  if (prom >= 7.5) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error generando el certificado." });
    }
  }
...
function generarCertificado(nombre, promedio) {
  ...
  doc
    .fontSize(16)
    .text(
      `CodeValidated se enorgullece de otorgar el presente certificado a:`,
      { align: "center" }
    );
  doc.moveDown();
  ...
  doc
    .fontSize(16)
    .text(`Por haber aprobado la Certificacion de React.js`, {
      align: "center",
    });
  doc.moveDown();
  doc
    .fontSize(14)
    .text(`Con un puntaje de ${promedio.toFixed(2)} puntos.`, {
      align: "center",
    });
  doc.moveDown(3);
  doc
  ...
  doc
    .fontSize(12)
    .text("Avalado por la Secretaría de Educación Pública (SEP).", {
      align: "center",
    });
...
const verifyDone = (req, res) => {
  ...
  let user = user.find((u) => u.cuenta === userId);
  ...
  return res.status(200).json({
    mensaje: "Usarion encontradio , valor de realizado enviado",
    usuario: { cuenta: user.cuenta, realizado: user.realizado },
  });
};

module.exports = { iniciarCertificado, enviarRespuestas, generarCertificado ,verifyDone};
